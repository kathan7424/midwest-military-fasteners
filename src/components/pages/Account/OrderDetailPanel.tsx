...
export default function OrderDetailPanel({
  ...
  return (
    <div>
      ...
      {/* Line items table */}
      {(() => {
        const specUrls = order.line_items
          .map((item) => map_product_spec_href(item.spec_file_url))
          .filter(Boolean);
        const certUrls = order.line_items
          .map((item) => map_product_spec_href(item.certificate_file_url))
          .filter(Boolean);

        return (
          <>
            {(specUrls.length > 1 || certUrls.length > 1) ? (
              <div className="mb-3 flex flex-wrap gap-2">
                {specUrls.length > 1 ? (
                  <button
                    type="button"
                    onClick={() => triggerBulkDownload(specUrls)}
                    className="inline-flex items-center gap-1.5 rounded-none border border-navy px-3 py-2 font-condensed text-xs font-bold uppercase text-navy transition-colors hover:bg-navy hover:text-white"
                  >
                    <DownloadCloud className="size-3.5" aria-hidden="true" />
                    Download All Spec Sheets ({specUrls.length})
                  </button>
                ) : null}
                {certUrls.length > 1 ? (
                  <button
                    type="button"
                    onClick={() => triggerBulkDownload(certUrls)}
                    className="inline-flex items-center gap-1.5 rounded-none border border-amber px-3 py-2 font-condensed text-xs font-bold uppercase text-amber transition-colors hover:bg-amber hover:text-white"
                  >
                    <DownloadCloud className="size-3.5" aria-hidden="true" />
                    Download All Certificates ({certUrls.length})
                  </button>
                ) : null}
              </div>
            ) : null}
