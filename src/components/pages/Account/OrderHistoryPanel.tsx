...
function triggerBulkDownload(urls: string[]): void {
  urls.forEach((url, i) => {
    setTimeout(() => {
...
function OrderTableSkeleton() {
  return (
    <div className="overflow-x-auto max-w-[1320px]" aria-busy="true">
      <table className="w-full border-collapse text-link [&_td]:align-middle [&_a]:align-middle">
        <thead>
          <tr className="bg-navy text-left text-white">
            ...
            <th className="px-4 py-3 text-center font-semibold uppercase">Certificate</th>
        ...
        <tbody>
          {[0, 1, 2].map((i) => (
            <tr key={i} className="border-t border-light-gray">
              ...
              <td className="px-4 py-3.5 text-center"><SkeletonBlock className="h-4 w-20 mx-auto" /></td>
      ...
      <table className="w-full border-collapse text-link [&_td]:align-middle [&_a]:align-middle">
        <thead>
          <tr className="bg-navy text-left text-white">
            ...
            <th className="px-4 py-3 font-semibold uppercase">Status</th>
            <th className="px-4 py-3 text-center font-semibold uppercase">Certificate</th>
            <th className="px-4 py-3 font-semibold uppercase w-[100px]">Total</th>
            <th className="px-4 py-3 font-semibold uppercase w-[100px]"><span className="sr-only">item</span></th>
            <th className="px-4 py-3 font-semibold uppercase text-center">Action</th>
          </tr>
        </thead>
        <tbody>
          {orders.map((order) => {
...
function OrderRow({
  ...
  return (
    <>
      {(() => {
        const certProxyUrls = (order.certificates ?? [])
          .map((url) => map_product_spec_href(url))
          .filter(Boolean);

        return (
          <tr className="border-b border-light-gray align-middle">
            ...
            <td className="whitespace-nowrap px-4 py-3.5 text-black">
              {order.date}
            </td>
            ...
            <td className="px-4 py-3.5 text-center">
              {certProxyUrls.length === 0 ? (
                <span className="text-mid-gray">—</span>
              ) : certProxyUrls.length === 1 ? (
                <a
                  href={certProxyUrls[0]}
                  download
                ...
                <button
                  type="button"
                  onClick={() => triggerBulkDownload(certProxyUrls)}
                  className="inline-flex items-center gap-1 text-amber hover:underline"
                >
                  <DownloadCloud className="size-3.5" aria-hidden="true" />
                  Download All ({certProxyUrls.length})
                </button>
              )}
            </td>
            <td className="whitespace-nowrap px-4 py-3.5 pr-0">
              <span className="font-bold text-dark-gray min-w-[80px]">{order.total}</span>
            </td>
            <td className="whitespace-nowrap px-4 py-3.5 pl-0">
