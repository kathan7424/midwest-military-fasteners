 ...
 *   Flat table layout per Figma: PART #, DESCRIPTION, ORDERED, Download.
 * Developer: KP-184
...
/** MM/DD/YY short date for the ORDERED column. */
function format_ordered_date(dateStr: string): string {
  if (!dateStr) return "";
...
interface FlatItem extends OrderDocumentLineItem {
  order_id: number;
  order_date: string;
}

function flatten_orders(orders: OrderDocumentsGroup[]): FlatItem[] {
          ...
          <tr className="bg-navy">
            {Array.from({ length: cols }, (_, i) => (
              <th key={i} className="px-4 py-3">
                <SkeletonBlock className="h-3 w-20 bg-navy/40" />
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {[0, 1, 2, 3].map((i) => (
            <tr key={i} className="border-t border-light-gray">
              ...
              <td className="px-4 py-3"><SkeletonBlock className="h-4 w-56" /></td>
...
export default function OrderDocumentsPanel({
  ...
  useEffect(() => {
    fetch_order_documents()
      // Guard the shape: an auth failure or WP error can resolve without an
      // orders array — never let undefined reach the .map render path.
      .then((response) => setOrders(Array.isArray(response?.orders) ? response.orders : []))
      .catch(() => setError("Unable to load your purchased product documents."))
      .finally(() => setIsLoading(false));
  }, []);

  if (isLoading) {
    return <TableSkeleton cols={filter === "all" ? 5 : 4} />;
  }
  ...
  // Filter orders/items to only those relevant to this tab.
  const filteredOrders = orders
    .map((order) => ({
      ...order,
      items: (order.items ?? []).filter((item) => {
        if (filter === "spec") return has_product_document(item.spec_file_url);
        if (filter === "certificate") return has_product_document(item.certificate_file_url);
        return has_product_document(item.spec_file_url) || has_product_document(item.certificate_file_url);
      }),
    }))
    .filter((order) => order.items.length > 0);

  const rows = flatten_orders(filteredOrders);
    ...
    <div className="overflow-x-auto max-w-[1320px]">
      <table className="w-full border-collapse text-link [&_td]:align-middle [&_a]:align-middle">
        ...
        <tbody>
          {rows.map((item, idx) => (
            <tr
              key={`${item.order_id}-${item.product_id}-${item.sku}-${idx}`}
            ...
            >
              ...
              <td className="whitespace-nowrap px-4 py-3 text-black">
                {format_ordered_date(item.order_date)}
              </td>
              {showSpec ? (
