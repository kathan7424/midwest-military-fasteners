...
const FALLBACK_BANNER = "https://dev-mmf-wp.pantheonsite.io/wp-content/uploads/2026/06/contact-banner.webp";
...
export default function ContactPage({ pageData }: Props) {
  const heading = pageData?.heading || "Contact Us";
  const subHeading = pageData?.sub_heading || "How can we help?";
  const bannerUrl = pageData?.banner_image?.url ?? FALLBACK_BANNER;
  const bannerAlt = pageData?.banner_image?.alt || "Contact banner";
