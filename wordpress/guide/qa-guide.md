...
- **Product image magnifier is side-by-side (desktop only)** — hovering the main image shows a lens rectangle tracking the cursor ON the image and a 450×300 zoom pane to the RIGHT with the 2.5× magnified region. Below the `lg` breakpoint there is no lens/pane — tapping opens the lightbox. The image scaling in place (old behavior) instead of a side pane IS a regression.
