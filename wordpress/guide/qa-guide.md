...
- **Payment Methods tab** shows WC payment tokens (WC source of truth). ALL Stripe customer lookups go through `WC_Stripe_Customer` (user option `_stripe_customer_id`) — the same identity the checkout gateway and WC Stripe's token sync use. NEVER read/write a custom user-meta key for the Stripe customer: that creates a SECOND Stripe customer, cards added via SetupIntent attach to it, and WC Stripe's token-sync filter silently strips those tokens from every list ("card added successfully" toast but the card never appears; saved-card payment fails with a customer/PM mismatch). If a card added BEFORE this fix is missing, it's attached to the orphaned duplicate customer — re-add it once.
- **Saved payment methods at checkout follow the WC Stripe setting** — WC → Payments → Stripe → "Enable saved payment methods". ON + logged in: saved cards appear as radio options (first preselected) with "Use a new payment method" below, and new-card entry gets a "Save payment information to my account for future purchases." checkbox. OFF or guest: plain card entry, no list, no checkbox. The locations response is transient-cached ~5 min — allow that after flipping the setting. Card data never touches the store either way (saved card sends the stored `pm_...` id; saving uses the gateway's `wc-stripe-new-payment-method` flag).
...
- **Checkout load failure (WP down or PHP fatal)** — when the checkout page can't load cart data from WooCommerce, it shows a red "Checkout unavailable" banner with the error message and a "Refresh page" button. It does NOT show a stuck skeleton loader. If you see an infinite skeleton on /checkout, the API proxy layer is broken — check Next.js logs and WP logs, not the frontend. The error banner includes a `(code N)` suffix when the proxy sent no message: code 0 = the browser never got a response (network error / client timeout); any other code is the proxy's HTTP status. The browser console also logs `[checkout] state load failed: status=...` — always capture both when reporting.
- **"Checkout unavailable" for logged-in users with a healthy backend (fixed)** — two root causes were fixed together. (1) Concurrent identical GETs share one deduplicated fetch, and a `Response` body can only be read once — React StrictMode's double-mounted checkout effect made the second reader see empty data and fail with the generic message while WP was perfectly healthy. Every deduped caller now receives `response.clone()` (`api-client.ts`). (2) WP's `rest_cookie_check_errors` rejects logged-in REST requests without an `X-WP-Nonce`, which the server-side proxy can never send; `mmf_headless_cookie_auth` now validates the `X-MMF-Proxy` shared secret FIRST and overrides that error (`wordpress/inc/auth.php` — requires Pantheon deploy). The proxy also has two-stage session recovery: stale WC nonce → retry keeping only the WP login cookie → if still 401/403, retry as a clean guest so the page always renders.
...
- **Spec sheet / certificate links force a real download** — they route through `/api/download?url=...` (same-origin proxy that sets `Content-Disposition: attachment`), so the browser downloads the file instead of opening it in a new tab. This applies EVERYWHERE: shop table, product detail, AND the My Account Spec Sheets / Certifications tabs + Order detail line items. The proxy ONLY accepts files hosted on the WP origin (403 for anything else) and only allowed file extensions (`pdf png jpg jpeg webp gif svg zip doc docx xls xlsx csv txt`). If a spec link opens in a tab instead of downloading, the proxy wrapper is missing; if the browser saves a `download.json` file, the proxy REJECTED the URL — check the file's host and extension against the allowlist.
- **My Account document dates are US format** — Certifications/Spec Sheets order dates render MM-DD-YY via `format_us_date()` (string-split, no timezone shift). The Orders table date follows the WP admin date format setting (`date_i18n`), which is a different, also-intentional behavior.
...
- **Product certifications are OPT-IN at checkout (SOW)** — these are PRODUCT certs (MFR C of C, material/process certs, test reports), totally separate from TAX exemption certificates. The checkout Order Summary shows an "Add certification — Free" checkbox per eligible item; ticking adds the product's $0.00 cert product to the WC cart (the order line IS the opt-in record). Certificates are delivered ONLY for opted-in items and ONLY after shipped/completed: the "certificates ready" email (with per-item download links + My Account link via `MMF_FRONTEND_URL`), the Certifications tab, order detail, and order history all check `mmf_item_cert_opted_in()`. No opt-in → no certificate anywhere, even if the product has a cert file (`order-documents.php`). Cart page + mini-cart never show cert add-ons as rows — the parent shows "✓ Certification included — Free", the count badge skips certs, and removing the parent removes its cert too. Product setup: physical product meta `_cert_product_id` → $0 cert product with `_is_cert_product` = 1 and `_certificate_file_url` on the physical product.
...
- **Lightbox is full-standard** — spinner until the full-size image loads (fade-in on load), top-left "2 / 5" counter, looping prev/next (arrows, ArrowLeft/ArrowRight keys, dots, mobile swipe ≥50px), adjacent slides preloaded, body scroll locked while open, Escape/backdrop closes, and closing SYNCS the main image + active thumbnail to the last viewed slide. Any of these missing is a regression.
